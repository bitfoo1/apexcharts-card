...
One caveat, and one improvement. ApexCharts 6 is bigger than 5, but this fork
...
```
$ wget https://github.com/bitfoo1/apexcharts-card/releases/download/v2.6.3/apexcharts-card.js
```
...
```yaml
resources:
  - url: /local/apexcharts-card.js?v=2.6.3
    type: module
```
...
**Since v2.6.2 (fork-only):** `before_now`, `after_now` and `raw` also drive the **legend** value, not just the header. The legend shows a series' current value, and for a series reaching into the future the last point is the end of the forecast rather than the value now — on a solar forecast that is midnight, so both readouts said `0` while the curve was correct. With `raw` the legend shows the entity's own reading like the header does, but it is redrawn only when the chart is, so it can lag the header by up to one `update_interval`.
