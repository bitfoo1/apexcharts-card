...
```
$ wget https://github.com/bitfoo1/apexcharts-card/releases/download/v2.6.1/apexcharts-card.js
```
...
```yaml
resources:
  - url: /local/apexcharts-card.js?v=2.6.1
    type: module
```
...
**Since v2.6.2 (fork-only):** `before_now` and `after_now` also drive the **legend** value, not just the header. The legend shows a series' current value, and for a series reaching into the future the last point is the end of the forecast rather than the value now — on a solar forecast that is midnight, so both readouts said `0` while the curve was correct.
